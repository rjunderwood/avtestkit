...
def make_window(theme):
    ...
    input_layout =  [
                ...
                # [sg.Menu(menu_def, key='-MENU-')],
                 [sg.Text('Run Test', size=(100, 1), justification='center', font=("Helvetica", 16), relief=sg.RELIEF_RIDGE)],
               [sg.Text('Run Scenario: ')], 
            [sg.Combo(values=("Default","Follow Vehicle"), default_value='Follow Vehicle', readonly=True, k='-SCENARIO SELECT-', auto_size_text=True, size=(100,20))],
        
            [sg.Text('Metrics:')], 
            
            [sg.Text('Speed:'),sg.Input(key='-INPUT-'),sg.Text('km/ph')],
            [sg.Text('Distance:'),sg.Spin([i for i in range(1,4)], initial_value=1, k='-SPIN-'),sg.Text('sec')],
